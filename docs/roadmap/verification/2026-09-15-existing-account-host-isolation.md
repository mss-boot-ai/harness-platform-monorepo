...
`36e7439...` 二进制 SHA-256：`54ac7509e1e9005e9924a26edeca35431ea564770844296b8073bcaf853a9e24`。完整 source SHA、探针代码及二进制一致。四组使用同一候选：

1. 有意保留“已持久关闭、尚未删除”的真实空 cgroup；释放旧实例后，由新进程读取状态并完成退休，没有启动 runtime。
2. 对已就绪的合成 Host 主进程注入 SIGKILL。先在另一 delegation root 尝试读取同一状态，确认拒绝且 registry 字节不变；再回到原 root 的新服务 invocation 核对旧 scope、确认关闭，未重新运行旧任务。
3. 同时运行两个独立 scope；关闭第一个后，第二个仍能完成真实 ACP 往返及自己的受限 relay 请求，然后分别完整关闭。
4. 真实 Codex 完成回复、文件读取、拒绝/批准单个合成文件、配置确认、实际 sleep 进程开始后取消、同会话继续、完整清理。

socketpair 现为正向白名单；目标内核验证了 stream/seqpacket 的允许 flags、peer 关闭前后的重定向拒绝，以及 DGRAM/RAW/不支持类型、域、协议和 flags 的 EPERM 拒绝。原始 scope 位置以 registry v2 在启动前持久绑定；缺少该绑定的 v1 文件保留并要求显式核对，不做猜测迁移。

四组继续比较现有账号清单与正式 ABA 的 MainPID/InvocationID，全部未变。测试结束后，仅停止本次命名的临时测试单元；证据文件与旧候选状态保留，不删除真实会话或运行记录。H2 数据库依赖只做了版本资料检查，未接入或初始化业务存储。
...
`40827a51fcac56573120978a25b8b3ac204a08e9` 的范围恢复、迁移拒绝和 peer 检查通过，但真实控制在取消阶段退出。内核日志确认每 Run 96 task 限额拒绝 fork；此前 ecb70c/ca0f35 的运行中也出现过该计数，不能将其历史功能通过等同于无资源拒绝。`36e7439...` 将默认单 Run 改为 192 task、服务改为 512（仍为 Host 预留 64），不移除限制；关闭时持久记录 pids/oom 拒绝计数，正常验收须明确为零。最终四组重跑通过，真实控制与正常 peer 场景计数为零。SIGKILL 场景不将已消失的内核计数伪造为零。

## 尚未建立的结论

本报告不把所有启动/存储/迁移切点、HC 公共入口升级验收、H2 sealed 事务存储、H3 离线执行服务、H4 同端点快照恢复、跨设备 Grant/lease、资源和 PWA 标为完成。仅以上明确列出的双 scope、Host-death、退休和私有管道场景具有对应检查点证据，不把后续代码自动沿用为已验收。
