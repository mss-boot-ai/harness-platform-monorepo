...
- 保真度：F4（还原稿，页面主体）／外壳为重建
- 界面：Platform Harness 业务五个页面的静态还原，加 403 与空/加载态，共 7 屏。
- 使用者：管理员 + 运维
...
| 左侧导航 / 顶栏 / 布局 / 主题 token | **重建**：实包为 `@mss-boot-io/admin-web 1.3.7`，不在仓库且本地未安装，无法读取 | **低：非逐像素** |
| 空态 / 错误 / 加载 / 403 组件外观 | 来自该包的 `PageEmpty` 等导出，外观重建 | 低：组件外观重建，分支逻辑源码级 |
...
| P2b | 验证用户码弹窗 | 同上 | 同上 |
| P3 | 端点管理 + 吊销确认 | `pages/Endpoints/index.tsx` | 同上 |
...
1. **侧栏只有一个 Harness 条目。** `routes.config.ts` 里除 `/harness/overview` 外，其余四页都是 `hideInMenu: true`，导航靠页面内的 **Tabs**（概览 / 注册审批 / 端点 / 会话 / 投递）完成。把五个页面画成五个侧栏菜单项是**错的**。
2. **概览是 5 个统计卡不是 4 个**：待处理注册、活跃端点、活跃会话、未确认密文帧、**冲突密文帧**。栅格是 `xs=24 sm=12 xl=8`。
...
1. **应用外壳非逐像素**（见 §1 最后两行）。侧栏宽度、品牌区、菜单字体、空态插画、顶栏高度等可能与生产界面不同。要消除差距需要在该目录安装依赖后重新对照实包。
2. **Foundation 菜单未还原**。`generated/routes.ts` 是空数组，Foundation 菜单由运行时包注入，源码中不可读，因此没有虚构菜单名，改用虚线框说明。
3. **示例数据为占位值**：端点名、ID、时间、Runtime / Workspace、字节数均按源码字段格式编造。
4. **标签色值为 antd 6 默认调色板重建**，不是从实包 CSS 读取。
5. **Popconfirm / Modal 用叠加层复现**，不响应点击。
6. **未覆盖**：真实登录跳转、请求头与 CSRF、`Idempotency-Key` 生成、`message.useMessage()` 全局提示动效、en-US 文案、表头排序与列宽拖拽。

## 5. 待确认问题

1. 侧栏宽度与是否深色，需安装依赖后从实包确认；当前为 `208px` 浅色重建。
2. 是否有必要把 Foundation 菜单按经验补全（会引入虚构成分），还是保持现状只标注不可读？
3. 「冲突密文帧」在概览里的业务含义与处置入口是否需要补充说明页面？
4. 投递页的 `Session ID` 输入是否需要历史下拉或从会话页带参跳转的可见提示？

## 6. 版本记录

| 版本 | 日期 | 变更 | 触发文档 / 决策 |
| --- | --- | --- | --- |
| v0.1 | 2026-09-15 | 首次建立：按真实 locales 与页面组件还原 7 屏；外壳标注为重建 | `platform/web/src/business` 当前源码 |
